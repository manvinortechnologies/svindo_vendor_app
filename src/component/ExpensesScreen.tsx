...
const ExpensesScreen = ({navigation}:any) => {
    ...
    const getAllExpenses=async()=>{
        try {
            setIsLoading(true)
            const res=await api.get("vendor/expense/");
            console.log("res--->",res)
...
setIsLoading(false)
    ...
    const renderExpenese = ({ item, index }: { item: Expense; index: number }) => {
          ...
          <View style={styles.expenseRow}>
            ...
            <Text style={styles.descText}>{item.amount}</Text>
             <Text style={styles.descText}>{item.payment_date}</Text>
              <Text style={[styles.descText,{textAlign:"right"}]}>{item.is_paid?"No":"Yes"}</Text>
          </View>
        </View>
      </View>
        )
  ...
  return (
    ...
    <View style={styles.container}>
        <Headerwithback 
        title='Expenses'
        rightIcons={  [  <Icon name="calendar-outline" size={22} color="#FCA311" />]}
        />
      {/* Header */}
