...
const Orders = () => {
    ...
    const statuses = ["All", "Pending", "Accepted", "Shipped", "Delivered", "Returned","Exchage", "Cancle"];
    const orderTypes = ["On Shop", "Self Pickup", "Instant", "General"];
    const filters = ["Price: Low to High", "Price: High to Low", "Newest First", "Oldest First"];
...
<ScrollView style={styles.midcontent}>
        ...
        <TouchableOpacity style={styles.filterButton} onPress={() => setFilterModalVisible(true)}>
            <Text style={styles.filterText}>{selectedFilter === "None" ? "Filters" : selectedFilter}</Text>
          ...
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              ...
              {filters.map((filter) => (
                ...
                >
                  <Text style={[styles.filterOptionText, selectedFilter === filter && styles.selectedFilterText]}>{filter}</Text>
                </TouchableOpacity>
              ))}
        ...
        renderItem={({ item }) => (
          <View style={styles.orderCard}>
            <View style={styles.orderHeader}>
              ...
              <Text style={styles.orderDate}>{item.date}{"\n"}{item.time}</Text>
            </View>
...
const styles = StyleSheet.create({
 ...
 paddingTop: Platform.OS === "android" ? StatusBar.currentHeight || 40 : 0,
  ...
  filterOption: { padding: 10, borderBottomWidth: 1, borderBottomColor: "#ddd" },
  selectedFilter: { backgroundColor: "#ffb347" },
  filterOptionText: { fontSize: 16, color: "#333" },
  selectedFilterText: { fontWeight: "bold", color: "#fff" },
  closeButton: { padding: 10, backgroundColor: "red", marginTop: 10, borderRadius: 5, alignItems: "center" },
