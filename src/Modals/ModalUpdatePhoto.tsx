...
const ModalUpdatePhoto: React.FC<ModalUpdatePhotoProps> = ({
  ...
  const requestGalleryPermission = async () => {
    ...
    try {
      ...
      if (result === RESULTS.BLOCKED || result === RESULTS.LIMITED) {
        Alert.alert('Permission Denied', 'Gallery access is required. Please enable it in settings.');
      }
      return false;
    } catch (error) {
  ...
  const handlePickerImage = async (target: 'camera' | 'library') => {
    try {
      ...
      Alert.alert('Error', `Failed to ${target === 'camera' ? 'capture' : 'select'} image.`);
    }
  };
      ...
      <View style={styles.overlay}>
        <View style={styles.modalContainer}>
          ...
          <View style={styles.buttonContainer}>
            <TouchableOpacity style={styles.button} onPress={() => handlePickerImage('camera')}>
              <Ionicons name="camera" size={24} color="#000" />
              <Text style={styles.buttonText}>Take Photo</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.button} onPress={() => handlePickerImage('library')}>
